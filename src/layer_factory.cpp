 ...
 * @Author: zzh
 * @Date: 2023-03-04
 * @LastEditTime: 2023-03-07 10:49:25
 * @Description: 
...
auto LayerRegister::CreateLayer(const std::string &layer_type, const std::shared_ptr<Operator> &op) -> std::unique_ptr<Layer> {
    Registry &registry = GetRegistry();
    ...
    std::unique_ptr<Layer> layer = creator(op);
    return layer;
}
