...
class Layer {
    public:
        ...
        * @param const std::vector<std::vector<std::shared_ptr<Tensor<float>>>> &input_blobs, 
        *        第一个vecotor表示多个输入, 第二个vector表示多个batch, shared_pter指向blob对应的Tensor
        * @param std::vector<std::vector<std::shared_ptr<Tensor<float>>>> &output_blobs, 
        *        第一个vector表示多个输出, 第二个vector表示多个batch 
        * @return {*}
        */
        virtual auto Forward(
            const std::vector<std::vector<std::shared_ptr<Tensor<float>>>> &input_blobs,
            std::vector<std::vector<std::shared_ptr<Tensor<float>>>> &output_blobs) const -> int = 0;

        // virtual auto ForwardInplace(std::vector<Tensor<float>> &blobs) const -> int; 
        auto LayerName() const -> const std::string & {
